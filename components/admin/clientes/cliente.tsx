...
export default function Cliente({ client }: ClientFormProps) {
  ...
  const handleDelete = async () => {
    if (!confirm(`¿Deseas eliminar el registro de ${name}?`)) return
    try {
      setIsDeleting(true)
      ...
      if (res && !res.message?.toLowerCase().includes('error')) {
        ...
        setToast({ message: res?.message || 'Error al eliminar el cliente', type: 'error' })
      }
    } catch {
      setToast({ message: 'Error de red al eliminar el cliente', type: 'error' })
    } finally {
      setIsDeleting(false)
    }
  }
