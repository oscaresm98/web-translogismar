...
export default async function EditarNosotros({ params }: { params: Promise<{ id: number }> }) {
  const { id } = await params
  const enterprise = await getEnterprise(id) as EnterpriseInterface

  if (Object.keys(enterprise).length === 1) redirect('/admin/nosotros')
